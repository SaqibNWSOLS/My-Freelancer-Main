...
if (! function_exists('catTotalBlog')) {

    function catTotalBlog($id){
     
      $childCat=SitePageCategory::where('page_categories_id',$id)->pluck('id')->toArray();
      
        $subChildCat=SitePageCategory::whereIn('page_categories_id',$childCat)->pluck('id')->toArray();

        $mainCat=[(int)$id];
        $totalPages = Blog::whereIn('page_categories_id',array_merge($mainCat,$childCat,$subChildCat))->count();
...
if (! function_exists('catTotalPage')) {

    function catTotalPage($id){
      ...
      $childCat=SitePageCategory::where('page_categories_id',$id)->pluck('id')->toArray();
        ...
        $mainCat=[(int)$id];
        $totalPages = SitePage::whereIn('page_categories_id',array_merge($mainCat,$childCat,$subChildCat))->count();
...
function generateIndex($html) {
    ...
    foreach ($matches[0] as $i => $match){
        ...
        $curr = (int)$matches[1][$i];
        $text = strip_tags($matches[2][$i]);
        $slug = (str_replace(" ", "-", preg_replace('/[^a-zA-Z1-9\s]/', '', $text)));
          $html = preg_replace('/title="[^"]*"([^>]*>)/', '', $html); // remove title //$plaintext = preg_match('#<title>(. *?) </title>#', $html);
         // echo $html; exit;
        $anchor = '<a name="'.$slug.'" >'.$text.'</a>';
        $html = str_replace($text,$anchor,$html);
        $prev <= $curr ?: $index .= str_repeat('</ul></div>',($prev - $curr));
        $prev >= $curr ?: $index .= "<div class='accordion-content'><ul class='menu-accordion'></li>";

        $index .= '<li class="accordion-header"><a  href="#'.$slug.'">'.$text.'</a>';
    ...
    function getUserInfo( $image_dimension = '60x60') {
        ...
        if( Auth::user() ){
            ...
            $info = array(
                ...
                'slug'          => isset($user_detail->slug)?$user_detail->slug:'',
            );
        }
        return $info;
    }
...
if( ! function_exists ('getHeaderMenu') ){
    ...
    function getHeaderMenu() {
        ...
        $header_menu = [];
        $menu =Menu::select('id')->where('location', 'header')->latest()->first();
        
        
        if( !empty($menu) ){
            $header_menu =  MenuItem::where('menu_id', $menu->id)->orderBy('sort','asc')->tree()->get()->toTree(); 
...
if( ! function_exists ('uploadImage') ){
    ...
    new Intervention\Image\Drivers\Gd\Driver()
                ...
                $uploaded_image = $manager->read($imageUrl)->resize($dimension['width'], $dimension['height'])->save($directoryUrl.'/'.$thubnail_file_name.$file_ext);
...
if( ! function_exists ('uploadFile') ){
    ...
    function uploadFile( $dirName, $imageUrl, $dimensions = [] ) {
