    ...
    <div class="border flex flex-col gap-5 shadow-sm">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
        ...
        <div class="col-span-1 md:col-span-1">
          <div class="mr-3">
            <p class="mb-3">Enter Old Password</p>
            <input v-model="old_password" class="p-2 w-full bg-gray-100 border border-gray-300 rounded-md" type="password" />
            <p class="mb-3 mt-4">Enter New Password</p>
            <div class="relative">
              <input v-model="password" :type="passwordVisibility['password'] ? 'text' : 'password'" class="p-2 w-full bg-gray-100 border border-gray-300 rounded-md" />
            ...
            <div class="relative">
              <input v-model="password_confirmation" :type="passwordVisibility['password_confirmation'] ? 'text' : 'password'" class="p-2 w-full bg-gray-100 border border-gray-300 rounded-md" />
              <span class="absolute inset-y-0 right-0 flex items-center pr-3">
            ...
            <p v-if="password && !isPasswordStrong" class="text-red-500 mt-2">Password must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter, one number, and one special character.</p>
            <div class="text-right">
              <button :disabled="!isPasswordConfirmed || !isPasswordStrong" @click="changePassword" class="px-3 py-2 mt-4 text-white font-semibold bg-blue-800 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-md">
                Change Password
              </button>
            </div>
          </div>
