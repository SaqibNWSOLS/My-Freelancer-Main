<template>
  <div class="w-full">
    ...
    <p class="text-sm">
      A 6-digit verification code has been sent to the number {{ phone }}
    </p>
  </div>
  <div class="w-full mt-4">
    <v-otp-input v-model="form.code" :num-inputs="6" class="otp-input"></v-otp-input>
  </div>
  <div class="w-full text-right mt-4">
    <button
      class="text-white bg-gray-400 py-2 px-4 rounded-md"
      :class="(form.code === 6) ? 'bg-green-500' : ''"
      :disabled="form.code !== 6"
      @click="check()"
    >
      Verify
