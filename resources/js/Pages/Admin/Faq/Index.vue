<template>
    <AdminLayout>
            ...
            <div>
                   <a  class="text-white btn btn-primary hover:-translate-y-1 hover:transition-all hover:ease-in-out" :href="route('faq.create')" style="float:right"><span class="mdi mdi-plus"></span> Add
</a>
            </div>
        </template>
                        ...
                        <table  ref="dataTable"  class="table equal-space head-table">
                            ...
                            <tbody>
                                <tr v-for="(faq, index) in faqs" :key="faq.id">
                                    ...
                                    <td class="min-w-[8.125rem]">{{ faq.category_detail?.name }}</td>
                                    <td class="min-w-[11.125rem]">{{ faq.question }}</td>
                                    <td class="min-w-[11.125rem]">{{ faq.answer }}</td>
                                    <td class="min-w-[11.125rem]">{{ faq.status }}</td>
                                    <td>
                                        
                                        <a :href="route('faq.edit', faq.id)" lass="hover:-translate-y-1 hover:transition-all hover:ease-in-out" style="margin:10px"><span class="mdi mdi-database-edit text-lg text-[green]"></span></a>
                                        <button @click="deleteFaq(faq.id)" class="hover:-translate-y-1 hover:transition-all hover:ease-in-out"><span class="mdi  mdi-delete-empty-outline text-lg text-[red]"></span></button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
...
export default {
    ...
    methods: {
        editCountry(id) {
            Inertia.get(route('faq.edit', id));
        },
        async deleteFaq(id) {
        if (confirm('Are you sure you want to delete this faq?')) {
            try {
                const response = await this.$inertia.delete(route('faq.destroy', id));
