            ...
            <div class="card">
                <div class="card-header border-b-0">
                    <div class="flex flex-col">
                        <h3 class="text-lg font-medium">Employer</h3>
                    </div>
                </div>
                        ...
                        <table  ref="dataTable"  class="table equal-space head-table">
                            ...
                            <tbody>
                                <tr v-for="(employee, index) in employees" :key="employee.id">
                                   ...
                                   <td>
                                    <a :href="route('employees.show',employee.id)"><span class="mdi mdi-view-agenda-outline text-md text-[green]"></span></a>
                                          <a :href="route('employees.edit', employee.id)" lass="hover:-translate-y-1 hover:transition-all hover:ease-in-out" style="margin:10px"><span class="mdi mdi-database-edit text-md text-[green]"></span></a>
                                        <button @click="deleteEmployer(employee.id)" class="hover:-translate-y-1 hover:transition-all hover:ease-in-out"><span class="mdi  mdi-delete-empty-outline text-md text-[red]"></span></button>
                                   </td>
                                </tr>
                            </tbody>
                        </table>
...
export default {
  ...
  async deleteEmployer(id) {
        if (confirm('Are you sure you want to delete this?')) {
            try {
                const response = await this.$inertia.delete(route('employees.destroy', id));
